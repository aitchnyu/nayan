<template>
  <div>
    ...
    >
      ...
      <!--            <a :href="`/issues/${selected.point.lat}/${selected.point.lng}`">Issues around {{ selected.name }}</a>-->
      <!--          </l-tooltip>-->
      <!--        </l-marker>-->
      <l-polygon
        :lat-lngs="bounds"
    ...
    <portal selector="#sidebar-stuff">
      <a :href="`/issues/${centerMirror.lat}/${centerMirror.lng}/create`">
        create
      </a>
      <a :href="recenterUrl">
        recenter
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      rawIssues: {type:String, required: true},
      rawBounds: {type:String, required: true}},
    data: function () {
      ...
      return {
        ...
        zoom: 17,
        bounds: JSON.parse(this.rawBounds),
        issues: JSON.parse(this.rawIssues).map(i => ({...i, slug: slugify(i.title)})),
